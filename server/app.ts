...
mongoose
  .connect(config.MONGO_DB_URI as string, {
...
io.on("connection", (socket) => {
  ...
  const { groupId, jwtToken } = socket.handshake.query;
  ...
  socket.on("message", async (msg: string) => {
    ...
    try {
      jwt.decode(jwtToken);
      io.to(groupId).emit("message", { content: msg, nick: "anon" });
    } catch (err) {
      io.broadcast.emit("error", err);
    }
  });
