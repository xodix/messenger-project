...
router.post("/addAdmin", auth, async (req: any, res) => {
  try {
    res
      .status(200)
      .json(
        await Group.addAdmin(req.body.groupId, req.id, req.body.newAdminId)
      );
  } catch (error) {
    res.status(400).json(error.message);
  }
...
router.delete("/deleteAdmin", auth, async (req: any, res) => {
  try {
    res
      .status(200)
      .json(
        await Group.removeAdmin(req.body.groupId, req.id, req.body.exAdminId)
      );
  } catch (error) {
    res.status(400).json(error.message);
  }
...
router.post("/addParticipant", auth, async (req: any, res) => {
  try {
    res
      .status(200)
      .json(
        await Group.addParticipant(
          req.body.groupId,
          req.id,
          req.body.newParticipantId
        )
      );
  } catch (error) {
    res.status(400).json(error.message);
...
router.delete("/deleteParticipant", auth, async (req: any, res) => {
  try {
    res
      .status(200)
      .json(
        await Group.removeParticipant(
          req.body.groupId,
          req.id,
          req.body.exParticipantId
        )
      );
  } catch (error) {
    res.status(400).json(error.message);
